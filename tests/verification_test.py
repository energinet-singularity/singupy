...
def test_dataframe_columns():
    ...
    dataframe_columns(testframe, ["col1"], allow_extra_columns=True)

    # Check raise error in case partial list, but 1:1 compare
    with pytest.raises(ValueError):
        dataframe_columns(testframe, ["col1"], allow_extra_columns=False)

    # Check raise error in case missing col in 1:1 compare
    with pytest.raises(ValueError):
        dataframe_columns(testframe, ["col1", "col3"], allow_extra_columns=False)

    # Check raise error in case missing col in partial compare
    with pytest.raises(ValueError):
        dataframe_columns(testframe, ["col1", "col3"], allow_extra_columns=True)

    # Check raise error in case missing col in partial compare
    with pytest.raises(ValueError):
        dataframe_columns(testframe, ["col2", "col1"], allow_extra_columns=False, fixed_order=True)
